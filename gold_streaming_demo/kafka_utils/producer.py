...
from gold_streaming_demo.config import GoldPriceConfig, KafkaConfig
...
def create_producer(config: KafkaConfig) -> KafkaProducer:
    ...
    try:
        logger.info(
            f"Creating Kafka producer with bootstrap servers: {config.bootstrap_servers}"
        )
...
def fetch_gold_data(config: GoldPriceConfig) -> pd.DataFrame:
    ...
    try:
        ...
        gold_data = yf.download(
            config.ticker_symbol, interval=config.interval, period=config.period
        )
        ...
        gold_df_sorted.columns = [
            "_".join(map(str, col)).strip() for col in gold_df_sorted.columns
        ]
        # 2. Optional: remove '_GC=F' from column names
        gold_df_sorted.columns = [
            col.replace("_GC=F", "") for col in gold_df_sorted.columns
        ]
...
def run_producer(fetch_interval: int = 60) -> None:
    ...
    try:
        ...
        logger.info(
            f"Starting continuous data fetching with {fetch_interval}s interval"
        )
